...
class WeatherHomeViewModel: ObservableObject {
    ...
    init() {
        ...
        self.majorCities = [MajorCity(name: "Gothenburg", coordinate: LocationCoordinate(latitude: 57.708870, longitude: 11.974560)),
                            MajorCity(name: "Stockholm", coordinate: LocationCoordinate(latitude: 59.334591, longitude: 18.063240)),
                            MajorCity(name: "Moutain View", coordinate: LocationCoordinate(latitude: 37.386051, longitude: -122.083855)),
                            MajorCity(name: "London", coordinate: LocationCoordinate(latitude: 51.509865, longitude: -0.118092)),
                            MajorCity(name: "New York", coordinate: LocationCoordinate(latitude: 40.730610, longitude: -73.935242)),
                            MajorCity(name: "Berlin", coordinate: LocationCoordinate(latitude: 52.520008, longitude: 13.404954))]
    }
    ...
    func loadWeatherData() {
        ...
        if let currentLat = StorageDefaults.lastSavedLocation?.latitude, let currentLong = StorageDefaults.lastSavedLocation?.longitude {
            WeatherManager.getWeatherData(latitude: currentLat, longitude: currentLong) { response, err in
                if response != nil, let lat = response?.coord.lat, let long = response?.coord.lon {
                    let currentCityWeatherData = MajorCityWeatherData(name: response?.name ?? "", coordinate: LocationCoordinate(latitude: lat, longitude: long), weatherData: response)
        ...
        for city in majorCities {
            ...
            WeatherManager.getWeatherData(latitude: city.coordinate.latitude, longitude: city.coordinate.longitude) { response, err in
                if response != nil {
                    let cityWeatherData = MajorCityWeatherData(name: city.name, coordinate: city.coordinate, weatherData: response)
